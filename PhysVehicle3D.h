...
public:
	...
	void Render();
	Cube CreateCarPart(const vec3 size, const vec3 position, const Color color);
	void ApplyEngineForce(float force, float maxVelocity);
	void Brake(float force);
	void Turn(float degrees);
	float GetKmh() const;
	float PhysVehicle3D::GetMass() const;
	void PhysVehicle3D::SetMass(float newMass);
